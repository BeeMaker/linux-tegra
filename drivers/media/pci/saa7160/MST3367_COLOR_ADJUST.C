...
void MST3367_ADJUST_CSC_TABLE( F42A18AF66EFF459eA332CA253FFF8A65 * pDevice, BYTE bright, BYTE contrast, BYTE saturation, BYTE hue, BYTE sharpness )
{
	...
	F966EAAB748564259849CBD3F3D6C48A7( KERN_INFO, "[%02d] MST3367_ADJUST_CSC_TABLE() bright(%d) contrast(%d) saturation(%d) hue(%d) sharpness(%d)\n", pDevice->F0C7DF8CDF1004a6eB97071CBDF459ECE, bright, contrast, saturation, hue, sharpness);
	...
	if( (tempbyte & 0x40) == 0) {
		...
		if( tempbyte == 0 ) {
			...
			E6FB1249C1F642A38BC0228741149CD8S_EX( pDevice, 0x00, 0x9C, 0x00, 0x93, p_sys_cfg->o_csc_original.b.data, 24 );
...
//	if(hue != 0x80)
	...
	#ifdef AME_MZ0380
		DBF051EEA3B648B49859145F02FA268CS_EX( pDevice, 0x00, 0x9C, 0x00, 0x93, p_sys_cfg->o_csc_matrix.b.data, 24 );
	#else
	int i = 0 ;
	for( i = 0;i < sizeof(ColorMatrix1TO1);i++ )
	{
...
VOID MST3367_SETUP_CSC_TABLE( F42A18AF66EFF459eA332CA253FFF8A65 * pDevice )
{
	...
	#ifdef AME_MZ0380
		DBF051EEA3B648B49859145F02FA268CS_EX( pDevice, 0x00, 0x9C, 0x00, 0x93, Table, 24 );
	#else
	int i = 0;
...
*/
	...
	MST3367_SetRegister( pDevice, 0x00, 0xB0, R00B0 | 0x20 ); // RX_OUTPUT_YUV422 / 08.BITS / EMBEDDED SYNC
